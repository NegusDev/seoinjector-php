...
Universal PHP library for managing SEO metadata from a centralized dashboard.
Works with **Laravel, WordPress, Symfony, CodeIgniter, or any PHP project**.
...
Instead of managing individual SEO entries per product or article, you can
define templates that automatically populate with your application's data.
...
| Challenge | Solution |
| --------- | -------- |
| Thousands of products, each needs unique SEO | Define template, supply product context |
| SEO metadata gets stale | Uses real-time product/article data |
| Managing meta tags per page is tedious | Centralized templates + auto rendering |
| Social sharing needs different OG tags | Dynamic OG generation from context |
...
$product = [
    ...
    'image' => '[https://example.com/iphone16pro.jpg](https://example.com/iphone16pro.jpg)',
    'price' => 999,
    'rating' => 4.8,
];
...
echo $metadata['og_image'];        // "[https://example.com/iphone16pro.jpg](https://example.com/iphone16pro.jpg)"
...
Explicitly set the language for metadata resolution (auto-detected from
Accept-Language header if not set).
...
```php
$seo = new SEOInjector('your_api_key', [
    'api_url' => '[https://api.seoinjector.com/api](https://api.seoinjector.com/api)', // Custom API endpoint
...
Contributions are welcome! Please see [CONTRIBUTING.md](https://www.google.com/search?q=CONTRIBUTING.md) for
guidelines.
